...
func _ready() -> void:
	# Keep processing even while the game is paused.
	process_mode = Node.PROCESS_MODE_ALWAYS

	# Pause the game as soon as the menu opens.
...
func _on_back_to_title_pressed() -> void:
	# Free player and return to title screen.
	SceneManager.transition_scene( "res://00_Global/Scenes/title_screen.tscn", "", Vector2.ZERO, "up" )
	get_tree().paused = false
